...
본 프로젝트는 센서 기반의 인간 행동 인식(HAR) 정확도를 향상시키기 위해, 전체 활동을 <mark>**정적(Static)**</mark>과 <mark>**동적(Dynamic)**</mark>으로 전략적으로 분리한 뒤, 각 활동 특성에 맞는 <mark>전용 모델을 별도로 학습</mark>하는 <mark>**2단계 모델링 전략(Two-Stage Modeling Strategy)**</mark>을 제안합니다.
